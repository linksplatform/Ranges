namespace Platform::Ranges
{
    struct Range<T> : IEquatable<Range<T>>
    {
        ...
        public: static implicit operator ValueTuple<T, T>(Range<T> range) { return (range.Minimum, range.Maximum); }

        public: static implicit operator Range<T>(ValueTuple<T, T> tuple) { return new Range<T>(tuple.Item1, tuple.Item2); }
